...
#include "display.h"
#ifdef __3DS__
extern void n3ds_bottom_draw(void);
extern void n3ds_bottom_touch(void);
extern void n3ds_stereo_present(void);
extern void n3ds_autokey_tick(void);
extern int  n3ds_stereo_init(void);
#endif
...
static void init_new_res_vars(int32_t davidoption)
{
    ...
    xdim = xres = surface->w;
#ifdef __3DS__
    /* Lower half belongs to the touch panel, so the engine's frame is half
       the surface. bytesperline below stays surface->w, which is what makes
       the two halves contiguous rows of one image. */
    ydim = yres = surface->h / 2;
#else
    ydim = yres = surface->h;
#endif
...
#ifdef __3DS__
	surface = SDL_SetVideoMode(w, h * 2, 8,
	                           sdl_flags | SDL_DUALSCR | SDL_FITWIDTH);
#else
...
void getvalidvesamodes(void)
{
    ...
    int stdres[][2] = {
#ifdef __3DS__
                        /* Native top screen first; 320x200 kept as a fallback
                           for anything that insists on a classic mode. */
                        {400, 240}, {320, 240}, {320, 200}
#else
