...
#include "control.h"
#ifdef __3DS__
extern void n3ds_input_update(void);
extern void n3ds_input_analog(ControlInfo *info);
#endif
#include "mouse.h"
#include "joystick.h"
...
#ifdef __3DS__
    /* Refresh the pad and let it write the analog axes. The mouse code below
       only adds to these fields, so on a console with no mouse attached it
       contributes nothing and the two coexist. */
    n3ds_input_update();
    n3ds_input_analog(info);
    return;
#endif
